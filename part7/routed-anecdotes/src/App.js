...
const AnecdoteList = ({ anecdotes }) => (
  <div>
    <h2>Anecdotes</h2>
    <ul>
      {anecdotes.map(anecdote => <li key={anecdote.id}><Link to={`/notes/${anecdote.id}`}>{anecdote.content}</Link></li>)}
    </ul>
  </div >
)
...
const CreateNew = (props) => {
  // Ahora el estado del formulario lo controla un hook personalizado
...
const App = () => {
  ...
  return (
    <div>
      ...
      <Routes>
        <Route path="/notes/:id" element={<Anecdote anecdote={anecdote} handleVote={vote} />} />
